...
def does_match_img_lbl(base_dir, split):
    ...
    if len(lbl_list)>len(img_list):
        ...
        np.savetxt(os.path.join(base_dir, f'{split}_lbl_without_img.txt'), arr_lbl_woimg, delimiter='\n')
        ...
        np.savetxt(os.path.join(base_dir, f'{split}_img_without_lbl.txt'), arr_img_wolbl, delimiter='\n')
