...
def does_match_img_lbl(base_dir):
    ...
    for sf in split_folders:
        ...
        if split in ['train', 'val', 'test']:
            ...
            if len(lbl_list)>len(img_list):
                ...
                np.savetxt(os.path.join(base_dir, f'{split}_lbl_without_img.csv'), arr_lbl_woimg, fmt='%s')
                ...
                np.savetxt(os.path.join(base_dir, f'{split}_img_without_lbl.csv'), arr_img_wolbl, fmt='%s')
